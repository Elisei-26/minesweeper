...
function leftClick(line, column) { 
    let arrayLine = new Array(1000).fill(0); // in this string I save the lines on which the boxes without bombs are
    let arrayColumn = new Array(1000).fill(0); // in this string I save the columns on which the boxes without bombs are
    let length = 0;
    ...
    for (let i = 0; i < length; ++i) { // go through the two strings
        ...
        for (let length2 = n + 2; n < length2; ++n) { // starting from here...
            if (table[m][n] == '4') {
                ++bombs;
            } 
        }
        for (let length2 = m + 2; m < length2; ++m) {
            if (table[m][n] == '4') {
                ++bombs;
            } 
        }
        for (let length2 = n - 2; n > length2; --n) {
            if (table[m][n] == '4') {
                ++bombs;
            } 
        }
